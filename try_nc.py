...
@np.vectorize(otypes=[bytes], cache=False)
def float_to_bytes(prob: float, rname: bytes):
    if prob == 0:
        return b""
    assert len(rname) == 6
    return rname + struct.pack(f"<f", prob)

vlen_suffix = ".vlenb"

def cvt_to_vlen(nii_path: Path):
    codec = VLenBytes()
    encoded_rname = nii_path.name.removesuffix(".nii.gz")
    nii = nib.load(nii_path)
    array = float_to_bytes(nii.dataobj, encoded_rname.encode("utf-8"))
    buf = codec.encode(array)

    output_fname = nii_path.with_suffix(vlen_suffix)
    with open(output_fname, "wb") as fp:
        fp.write(buf)
    return output_fname

def combine_vlen(vlenpaths:tuple[Path]):

    if len(vlenpaths) == 1:
        return vlenpaths[0]
    
    codec = VLenBytes()
    vlenpath1, vlenpath2 = vlenpaths
    
    arr1 = codec.decode(vlenpath1.read_bytes())
    arr2 = codec.decode(vlenpath2.read_bytes())
    arr_ = arr1 + arr2
    buf = codec.encode(arr_)

    vlenpath_ = vlenpath1.parent / Path(hashlib.md5((vlenpath1.name + vlenpath2.name).encode()).hexdigest()[:6]).with_suffix(vlen_suffix)
    vlenpath_.write_bytes(buf)
    return vlenpath_

def local_file(_sp: Path):
        ...
        with Pool(4) as proc:
            vlenb_files = list(
                tqdm(
                    ...
                    desc="Converting to VLENB",
                    total=len(filenames)
                )
            )
        
        print("converted all nii to vlen")

        while len(vlenb_files) > 1:
            with Pool(4) as proc:
                vlenb_files = list(
                    tqdm(
                        proc.imap(
                            combine_vlen,
                            batched(vlenb_files, 2)
                        ),
                        desc=f"Combinging to VLENB",
                        total=len(vlenb_files) // 2
                    )
                )

        vlenb_file = vlenb_files[0]

        idx_name = _sp

        buf = vlenb_file.read_bytes()
        idx_name.write_bytes(buf)

        metadataname = idx_name.with_suffix(".metadata.json")

        # TODO get this info from nii file
        metadataname.write_text(json.dumps({
            "shape": [256,256,256],
            "continguous": "F"
        }))

        return idx_name, metadataname
