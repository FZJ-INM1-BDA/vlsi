...
#         nii_combined: np.ndarray = None

#         for f in filenames:
#             encoded_rname = f.name.removesuffix(".nii.gz")
#             nii: nib.Nifti1Image = nib.load(f)

#             nii_mapped = float_to_bytes(nii.dataobj, f.name.encode("utf-8"))
#             if nii_combined is None:
#                 nii_combined = nii_mapped
#             else:
#                 nii_combined += nii_mapped

#         shape = nii_combined.shape
#         nii_combined = nii_combined.reshape(-1, order="C")
#         codec = VLenBytes()
#         output = codec.encode(nii_combined)
#         with open(idx_name, "wb") as fp:
#             fp.write(output)
...
def test_written_spatial_index(pos, expected):
    idx_file = Path("foovlenb")

    if not idx_file.exists():
        pytest.skip("skipping numcodec profiling tests")
        return

    buf  = idx_file.read_bytes()

    codec = VLenBytes()
    array = codec.decode(buf)

    shape = (256, 256, 256)

    # decoded vlenarray is a 1d array...
    assert len(array.shape) == 1
    # ... but should equal to the product of shape
    assert array.shape[0] == np.prod(shape)

    reshaped = np.reshape(array, shape=shape, order="F")

    for _pos, ex in zip(pos, expected):
        b = reshaped[tuple(_pos)]
        decoded_d = {}
        print("debug?", b, reshaped[tuple(_pos[::-1])], _pos, ex)
        while len(b) > 0:
            _b = b[:10]
